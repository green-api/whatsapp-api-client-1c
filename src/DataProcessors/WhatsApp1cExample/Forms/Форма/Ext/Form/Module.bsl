...
&НаКлиенте
Процедура ПодтвердитьПолученноеСообщениеЧерезВебхук(Команда)
	
	Если Не ЗначениеЗаполнено(deliveryTagТекущий) Тогда
		Сообщить("Перед подтверждением сообщения, его нужно сначала получить");
		Возврат;
	КонецЕсли;
	
	Если УдалитьВебхук(IdInstance, ApiToken, deliveryTagТекущий) Тогда
		Сообщить("Сообщение подтверждено");
		deliveryTagТекущий = Неопределено;
	Иначе
		Сообщить("Сообщение не подтверждено");
	КонецЕсли;
