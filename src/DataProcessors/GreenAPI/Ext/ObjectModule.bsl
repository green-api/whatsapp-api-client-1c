...
Функция ПолучитьВходящиеСтатусы(КоличествоМинут) Экспорт
	...
	Адрес   = URLМетодаСервисаПараметр(Инстанс, Токен, "GetIncomingStatuses", "minutes", Формат(КоличествоМинут, "ЧГ=0"));
	Ответ   = ОтправитьGETЗапрос(ХостПоУмолчанию(), Адрес, Истина);
...
Функция ПолучитьИсходящиеСтатусы(КоличествоМинут) Экспорт
	...
	Адрес   = URLМетодаСервисаПараметр(Инстанс, Токен, "getOutgoingStatuses", "minutes", Формат(КоличествоМинут, "ЧГ=0"));
	Ответ   = ОтправитьGETЗапрос(ХостПоУмолчанию(), Адрес, Истина);
...
// Подробнее https://green-api.com/docs/api/service/EditMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
//  ИдСообщения - Строка - Идентификатор удаляемого сообщения
...
Функция ПараметрИдЧата(Телефон) Экспорт
