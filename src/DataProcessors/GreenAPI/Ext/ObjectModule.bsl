...
Функция ОтправитьВидеоАудиоИзображениеДокумент(Телефон, ПутьКФайлу, Сообщение) Экспорт
	...
	// Необязательный параметр, если не передавать, то заголовок будет браться из названия файла
	ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель);
...
Функция УстановитьФотоПрофиля(ПутьКФайлу) Экспорт
	...
	ЗаписьТекста.ЗаписатьСтроку("Content-Type: " + ПолучитьMimeTypeПоРасширению(ОтправляемыйФайл.Расширение));
	ЗаписьТекста.ЗаписатьСтроку("");
	...
	Ответ = ОтправитьФайлPOSTЗапрос(Хост, URLМетодаСервиса(Инстанс, Токен, "setProfilePicture"), Заголовки, ФайлОтправки);
