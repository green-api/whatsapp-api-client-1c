...
&НаКлиенте
Функция ПолучитьУведомление(Хост, Инстанс, Токен)
	
	Ответ = ОтправитьGETЗапрос(Хост, URLМетодаСервиса(Инстанс, Токен, "receiveNotification"), Истина);
	Возврат Ответ;
...
&НаКлиенте
ПРоцедура ПолучитьСообщение(Инстанс, Токен)
	...
	Пока Истина Цикл
		Ответ = ПолучитьУведомление(ХостПоУмолчанию(), Инстанс, Токен);
		Если Ответ <> Неопределено Тогда
			...
			ИначеЕсли Ответ.body.typeWebhook = ТипыУведомлений.СостояниеТелефона Тогда
				...
				КонецПопытки;
			Иначе
				УдалитьСообщение(Инстанс, Токен, ТегСообщения); // Другие вебхуки тихо пропускаем
			КонецЕсли;
			
			Если Не Отказ Тогда
				УдалитьСообщение(Инстанс, Токен, ТегСообщения);
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
// Обработчик, срабатывающий при получении текствого, видео и т.п. медиа сообщения 
...
Процедура ПриПолученииСообщения(Отказ, Данные)
	...
	Если Данные.body.messageData.typeMessage = "textMessage" Тогда
		ТекстПолученноеСообщение = Данные.body.messageData.textMessageData.textMessage;
	ИначеЕсли Данные.body.messageData.typeMessage = "extendedTextMessage" Тогда
		ТекстПолученноеСообщение = Данные.body.messageData.extendedTextMessageData.text;
	ИначеЕсли Данные.body.messageData.typeMessage = "imageMessage"
		Или Данные.body.messageData.typeMessage = "videoMessage"
		Или Данные.body.messageData.typeMessage = "documentMessage"
		Или Данные.body.messageData.typeMessage = "audioMessage" Тогда
		
		ТекстПолученноеСообщение = Данные.body.messageData.fileMessageData.downloadUrl;
	ИначеЕсли Данные.body.messageData.typeMessage = "locationMessage" Тогда
		ТекстПолученноеСообщение = "latitude=" + Данные.body.messageData.locationMessageData.latitude 
	...
	ИначеЕсли Данные.body.messageData.typeMessage = "contactMessage" Тогда
		ТекстПолученноеСообщение = Данные.body.messageData.contactMessageData.vcard;
	Иначе
		ТекстПолученноеСообщение = Данные.body.messageData;
	КонецЕсли;
...
Процедура ПриПолученииСтатусаОтправленногоСообщения(Отказ, Данные)
	...
	ТекстПолученноеСообщение = Данные.body.status;
...
Процедура ПриПолученииСтатусаАвторизацииАккаунта(Отказ, Данные)
	...
	ТекстПолученноеСообщение = Данные.body.stateInstance;
...
Процедура ПриПолученииСтатусаУстройства(Отказ, Данные)
	...
	ТекстПолученноеСообщение = Данные.body.deviceData.deviceModel;
...
Процедура Подключаемый_ПолучитьСообщение() Экспорт
	...
	ПолучитьСообщение(IdInstance, ApiToken);
