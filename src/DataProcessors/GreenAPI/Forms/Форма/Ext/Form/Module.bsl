...
Процедура ПолучатьВходящиеУведомленияПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "webhookUrl", webhookUrl);
	ОбновитьСтатусСервиса();
...
Процедура ОтправитьСообщениеWhatsApp()
	ОбработкаОбъект().ОтправитьСообщениеWhatsApp(IdInstance, ApiToken, НомерТелефона, ТекстСообщения);
КонецПроцедуры
...
Процедура ОбновитьСтатусСервиса()
	...
	СтатусСервиса = ОбработкаОбъект().ПолучитьСтатусСервиса(IdInstance, ApiToken);
	
	Элементы.НастройкиВходящихУведомлений.Видимость = СтатусСервиса;
	
	Если СтатусСервиса Тогда
		НастройкиСервиса = ОбработкаОбъект().ПолучитьНастройкиСервиса(IdInstance, ApiToken);
		...
		ЕстьНестадантныенастройки = ЗначениеЗаполнено(НастройкиСервиса.webhookUrl) И СтрНайти(НастройкиСервиса.webhookUrl, ОбработкаОбъект().ХостВебхуковПоУмолчанию()) = 0;
...
Процедура ПолучатьВходящиеСообщенияИФайлыПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "incomingWebhook", 
...
Процедура ПолучатьСтатусыОтправленныхСообщенийПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "outgoingWebhook", 
		? (ПолучатьВходящиеСообщенияИФайлы, "yes", "no"));
...
Процедура ПолучатьУведомленииОСостоянииТелефонаПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "deviceWebhook", 
		? (ПолучатьВходящиеСообщенияИФайлы, "yes", "no"));
...
Процедура ПолучатьУведомленияОСостоянииАккаунтаПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "stateWebhook", 
		? (ПолучатьВходящиеСообщенияИФайлы, "yes", "no"));
...
Процедура ОтмечатьВходящиеСообщенияПрочитаннымиПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "markIncomingMessagesReaded", 
...
Процедура ИнтервалОтправкиСообщенийПриИзмененииСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "delaySendMessagesMilliseconds", ИнтервалОтправкиСообщений);
...
Процедура ПолучитьСообщениеЧерезВебхукСервер()
	...
	ОбработкаОбъект.ПолучитьСообщение(IdInstance, ApiToken);
...
Процедура УстановитьНастройкуСервисаСервер()
	...
	НастройкиСохранены = ОбработкаОбъект().УстановитьНастройкуСервиса(IdInstance, ApiToken, "webhookUrl", ОбработкаОбъект().ХостВебхуковПоУмолчанию());
	ОбновитьСтатусСервиса();
